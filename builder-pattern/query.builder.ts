...
export class QueryBuilder {
    ...
    build(){
        if (!this.query.select || !this.query.from) throw new Error("SELECT and FROM clauses are mandatory.");
        ...
        if (this.query.where?.length) parts.push("WHERE " + this.query.where.join(" AND "))
        if (this.query.groupBy?.length) parts.push("GROUP BY " + this.query.groupBy.join(", "))
        if (this.query.orderBy?.length) parts.push("ORDER BY " + this.query.orderBy.join(", "))
        return parts.join("\n");
